...
import { ChevronLeft, Plus, Edit2, Pill, Activity, CalendarDays, PlusCircle, MinusCircle, RefreshCw, Camera } from 'lucide-react';
...
export default function PatientDetails() {
  ...
  // Edit State
  const initEdit = searchParams.get('edit') === 'true';
  ...
  const handleCancelEdit = async () => {
    if (isNewUser) {
      // If they cancel creating a new user, delete the temporary record
  ...
  const handleQuickRefill = async (medId: string, currentCount: number, e: React.MouseEvent) => {
    ...
    if (refillMode === medId) {
       ...
       // Enter mode
       setRefillMode(medId);
       setRefillAmount(30); // Default to a standard 30 day supply
    }
  };

  if (patient === undefined) return <div className="p-6">Loading...</div>;
  if (patient === null) return <div className="p-6 flex flex-col items-center pt-24"><p className="text-gray-500 mb-4">Patient not found</p><button onClick={() => navigate(-1)} className="text-[#606C38] font-bold">Go back</button></div>;
    ...
    <div className="min-h-screen max-w-lg mx-auto bg-[#FBFBF8] pb-24 text-[#283618] font-sans">
      <div className="bg-white p-6 pt-12 pb-6 shadow-sm border-b border-[#E5E1D8] flex items-center justify-between sticky top-0 z-10">
        ...
        <h1 className="text-lg font-bold tracking-tight truncate flex-1 text-center px-4">
          {patient.designation ? `${patient.designation} ` : ''}{patient.first_name} {patient.last_name}
        </h1>
        <button 
          onClick={handleEditClick}
          className={`w-10 h-10 transition flex items-center justify-center rounded-full ${isEditing ? 'bg-[#606C38] text-white' : 'text-[#606C38] opacity-70 hover:opacity-100 hover:bg-[#F2F0E4]'}`}
        >
      ...
      <div className="p-6 space-y-6">
        ...
        {/* Patient Core Summary or Edit Form */}
        {isEditing ? (
          <div className="bg-white rounded-[32px] border border-[#E5E1D8] p-6 shadow-sm space-y-4">
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                {editForm.photo ? (
                  <img src={editForm.photo} alt="Profile" className="h-20 w-20 rounded-2xl object-cover shadow-sm border-2 border-[#E5E1D8]" />
                ) : (
              ...
              <p className="text-[10px] uppercase font-bold text-[#606C38] opacity-70 mt-3 tracking-wider">Patient Photo</p>
            </div>
            ...
            <div>
              ...
              />
              <p className="text-[10px] text-[#606C38] opacity-70 mt-1 font-bold">This PIN is required for the patient to log in.</p>
            </div>
            <div>
          ...
          <div className="bg-white rounded-[32px] border border-[#E5E1D8] p-6 shadow-sm">
            <div className="flex gap-4 items-center">
               {patient.photo ? (
                 <img src={patient.photo} alt={patient.first_name} className="w-14 h-14 rounded-full object-cover shadow-sm border-2 border-[#E5E1D8] shrink-0" />
               ) : (
               ...
               <div className="flex-1">
                  ...
                  <p className="text-xs text-[#606C38] opacity-70 mt-1">Managing: <span className="font-bold">{patient.designation ? `${patient.designation} ` : ''}{patient.first_name}</span> • DOB: {patient.dob}</p>
               </div>
            </div>
            {patient.notes && (
        ...
        <div className="flex bg-[#F2F0E4] border border-[#E5E1D8] p-1 rounded-2xl">
          <button 
            ...
            className={`flex-1 py-2.5 text-xs font-bold rounded-xl transition ${activeTab === 'meds' ? 'bg-white shadow-sm text-[#283618] border border-[#E5E1D8]' : 'text-[#606C38] opacity-70 hover:opacity-100'}`}
          >
            Medications
          </button>
          <button 
             onClick={() => setActiveTab('logs')}
            className={`flex-1 py-2.5 text-xs font-bold rounded-xl transition ${activeTab === 'logs' ? 'bg-white shadow-sm text-[#283618] border border-[#E5E1D8]' : 'text-[#606C38] opacity-70 hover:opacity-100'}`}
          >
            History Logs
          </button>
        </div>
        ...
        {activeTab === 'meds' ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => navigate(`/caregiver/patient/${id}/medication/new`)} className="flex items-center gap-1 xl text-xs text-white bg-[#606C38] px-3 py-2 rounded-lg font-bold hover:opacity-90 transition shadow-sm">
            ...
            {medications.length === 0 ? (
              ...
              medications.map(med => (
                <div key={med.id} onClick={() => navigate(`/caregiver/patient/${id}/medication/${med.id}`)} className="bg-white p-5 rounded-[32px] shadow-sm border border-[#E5E1D8] cursor-pointer hover:border-[#606C38] hover:shadow-md transition group relative">
                  ...
                  <div className="flex items-center gap-4 mb-4 pr-6">
                     ...
                     <div className="flex-1">
                       ...
                       <p className="text-xs text-[#606C38] opacity-70 font-semibold">{med.dosage} • {med.frequency || 'Daily'} {med.timing ? `(${med.timing.split(',').join(', ')})` : ''}</p>
                     </div>
                  </div>
                     ...
                     <div className="text-right flex items-center gap-2">
                       {refillMode === med.id ? (
                         <div className="flex items-center gap-1 bg-white p-1 rounded-xl shadow-sm border border-[#E5E1D8]">
                            <button onClick={(e) => { e.stopPropagation(); setRefillAmount(prev => Math.max(0, prev - 10)); }} className="p-1 text-[#606C38] hover:bg-[#F2F0E4] rounded-lg transition"><MinusCircle className="w-4 h-4"/></button>
                            <span className="font-bold text-sm text-[#283618] w-8 text-center">+{refillAmount}</span>
                            <button onClick={(e) => { e.stopPropagation(); setRefillAmount(prev => prev + 10); }} className="p-1 text-[#606C38] hover:bg-[#F2F0E4] rounded-lg transition"><PlusCircle className="w-4 h-4"/></button>
                         </div>
                       ) : (
                       ...
                       <button
                         ...
                         className={`ml-2 p-2 rounded-xl transition shadow-sm border ${refillMode === med.id ? 'bg-[#606C38] text-white border-[#606C38]' : 'bg-white text-[#606C38] border-[#E5E1D8] hover:bg-[#F2F0E4]'}`}
                       >
