...
DROP POLICY IF EXISTS "Allow anon uploads to medicore photos" ON storage.objects;
DROP POLICY IF EXISTS "Allow anon reads to medicore photos" ON storage.objects;
DROP POLICY IF EXISTS "Allow anon deletes to medicore photos" ON storage.objects;
DROP POLICY IF EXISTS "Allow anon updates to medicore photos" ON storage.objects;

CREATE POLICY "Allow anon uploads to medicore photos"
  ON storage.objects FOR INSERT TO anon
  WITH CHECK (bucket_id = 'medicore-photos');

CREATE POLICY "Allow anon reads to medicore photos"
  ON storage.objects FOR SELECT TO anon
  USING (bucket_id = 'medicore-photos');

CREATE POLICY "Allow anon deletes to medicore photos"
  ON storage.objects FOR DELETE TO anon
  USING (bucket_id = 'medicore-photos');

CREATE POLICY "Allow anon updates to medicore photos"
